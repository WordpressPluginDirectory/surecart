...
const ScCancelDiscount = class {
    ...
    render() {
        ...
        return (h("div", { key: '120482a397bce6fde3cdadba608fe6c3855c466f', class: "cancel-discount" }, h("sc-dashboard-module", { key: '346bfdbab0996bd52dcfd7ea093b19512617a70a', heading: title, style: { '--sc-dashboard-module-spacing': '2em' } }, h("span", { key: 'f1a4e1bb0b997f75bcc58c378454dbb5d76d8561', slot: "description" }, description), h("sc-flex", { key: 'c3d86ecfa08d70809ff0310e64369f499f67f4da', justifyContent: "flex-start" }, h("sc-button", { key: 'befb32eb8c647e387f6ad0f7548a26c15b84ff88', type: "primary", onClick: () => this.addDiscount() }, button), h("sc-button", { key: '8bf48199193ea2b65d1789e7a57d27f300db6a6a', class: "cancel-discount__abort-link", type: "text", onClick: () => this.scCancel.emit() }, cancel_link)), !!this.loading && h("sc-block-ui", { key: '97f97b49cf9dd471157abe78dc627cf9c73690a6', spinner: true }))));
    }
};
...
const ScSubscriptionCancel = class {
    ...
    render() {
        return (h("sc-dashboard-module", { key: 'f0230ba9e84806ac1190af7bafaa1389ded64fd0', heading: this.heading || wp.i18n.__('Cancel your plan', 'surecart'), class: "subscription-cancel", error: this.error, style: { '--sc-dashboard-module-spacing': '1em' } }, this.renderContent(), h("sc-flex", { key: 'b1d7bfe567ca847dddc0eecf2b927ddff454b421', justifyContent: "flex-start" }, h("sc-button", { key: '1a91e86325a3581687a01d8b69a52a09d4f12f28', type: "primary", loading: this.loading || this.busy, disabled: this.loading || this.busy, onClick: () => this.cancelSubscription() }, wp.i18n.__('Cancel Plan', 'surecart')), h("sc-button", { key: '63eccba98b8f026efbdda561763c7c4f02b8154b', style: { color: 'var(--sc-color-gray-500' }, type: "text", onClick: () => this.scAbandon.emit(), loading: this.loading || this.busy, disabled: this.loading || this.busy }, wp.i18n.__('Keep My Plan', 'surecart'))), this.busy && h("sc-block-ui", { key: '2763d7ebb12a5515b24e3497730e27d21bf6ec9d' })));
    }
};
